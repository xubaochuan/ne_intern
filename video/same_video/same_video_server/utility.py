...
r0 = redis.Redis(host='localhost', port=11103, db=0)
cover_rate = 0.5
...
def get_likehood_ids(vid, hashlist):
    ids_dict = {}
    likehood_ids = []
    number = len(hashlist)
    for hashcode in hashlist:
        ...
        if isExists:
            ...
            for id in ids:
                ...
                vid_timestamps = int(items[1])
                if now_timestamps - vid_timestamps > 86400 * 180:
                    continue
...
def video_pushin(r_db, vid, vtitle, imghash_list):
    now_timestamps = ";" + str(int(time.time()))
    if not r_db.exists(vid_prefix + vid):
        ...
        for imghash in imghash_list:
            ...
            r_db.rpush(imghash_prefix + imghash, vid + now_timestamps )
#            print "-------put key:%s value:%s" % (imghash_prefix + imghash, vid + now_timestamps)
...
#        return {'code':400, 'rvideoid':vid,'msg':vid + ' ' + vtitle + " had detected"}
    ...
    likehood_ids = get_likehood_ids(vid, imghash_list)
    print likehood_ids
    flag = 0
